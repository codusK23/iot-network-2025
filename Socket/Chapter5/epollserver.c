...
int main(int argc, char* argv[])
{
	...
	ev.events = EPOLLIN;		// epoll 이벤트 구조체의 events에 발생 이벤트 저장
	...
	while(1){
		/* 3. 이벤트 발생 대기, 인스턴스에 이벤트가 발생하면 이벤트 발생 정보를 events 배열에 저장  */
		event_cnt = epoll_wait(epfd, events, MAX_EVENTS, -1);	// 이벤트 발생 수 리턴
		if(event_cnt == -1){
		...
		for(int i = 0; i < event_cnt; i++){
			if(events[i].data.fd == server_fd){
				...
				epoll_ctl(epfd, EPOLL_CTL_ADD, client_fd, &ev);		// epoll 인스턴스에 추가
				printf("connected client: %d\n", client_fd);
			} else{
				// 발생한 이벤트가 클라이언트 fd 이면
				char buffer[1024] = {0};
